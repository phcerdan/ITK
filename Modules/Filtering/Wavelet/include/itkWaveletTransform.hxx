...
#ifndef itkWaveletTransform_hxx
#define itkWaveletTransform_hxx
...
::GenerateData()
{
  ...
  itkDebugMacro(<<"Allocating " << (1 + GetNumberOfDecompositions() * (filter->GetNumberOfOutputs() - 1)) << " output\n");
  this->GetOutput()->Resize(
    1 + GetNumberOfDecompositions() * (filter->GetNumberOfOutputs() - 1));

  itkDebugMacro(<<"Using " << this->GetOutput()->Size() << " outputs...");
...
::GenerateData()
{
  ...
  itkDebugMacro(<< "Found " << m_NumberOfDecompositions << " decompositions");
